...
case class Enum() extends FunSuite{
  ...
  test("extends value in enum"){
    object Movies extends Enumeration {
      ...
      val Musical = Movie( "movies with a lot of music", 1010, 4.05)
      val Comedy = Movie( "entertainment audience", 999, 4.7)
      val Drama = Movie( "emotional and unexpected series of events", 1022, 3.85)
      val Horror = Movie( "scare audience", 777, 4.1)

      sealed case class Movie private[Movies](describe: String, count: Int, mark: Double) extends Val(describe)
    ...
    def isLowestSet(movie: Movies.Movie): Boolean = {
      Movies.values.toList.minBy(t => t.count) === movie
    }
